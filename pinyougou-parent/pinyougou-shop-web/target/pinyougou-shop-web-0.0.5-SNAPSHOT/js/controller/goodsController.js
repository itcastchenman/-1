...
app.controller('goodsController' ,function($scope,$controller,$location   ,goodsService,uploadService,itemCatService,typeTemplateService){	
	...
	$scope.findOne=function(){	
		...
		goodsService.findOne(id).success(
			function(response){
				...
				editor.html($scope.entity.goodsDesc.introduction);//回显富文本
				$scope.entity.goodsDesc.itemImages=JSON.parse($scope.entity.goodsDesc.itemImages);//回显图片
				$scope.entity.goodsDesc.customAttributeItems=JSON.parse($scope.entity.goodsDesc.customAttributeItems);//回显扩展属性
				//回显规格
	...
	$scope.save=function(){	
		...
		$scope.entity.goodsDesc.introduction=editor.html();//将富文本内容存进来
	...
	*/
	...
	//读取二级分类
     ...
     $scope.$watch('entity.goods.typeTemplateId',function(newValue,oldValue){
    	 ...
    	 //查询规格列表($scope.specList={options:options[specId:id,optionName:optionName],text:text}
	...
	//创建 SKU 列表
	 $scope.createItemList=function(){
		 $scope.entity.itemList=[{spec:{},price:0000,num:9999,status:'0',isDefault:'0'}];//初始
	 ...
	 addColumn=function(list,columnName,columnValues){
		 ...
		 for(var i=0;i<list.length;i++){
			 ...
			 for(var j=0;j<columnValues.length;j++){
				 newRow=JSON.parse(JSON.stringify(oldRow));//深克隆
